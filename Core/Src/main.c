...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  HAL_ADC_Start_DMA(&hadc1,(uint32_t*)xy_val,4);
  /* USER CODE END 2 */
  ...
  /* add events, ... */
  Queue1Handle = osMessageQueueNew(20, sizeof(uint8_t), &Queue1_Attributes);
...
/* USER CODE BEGIN 4 */
void MX_ADC1_Init_BareMetal(void)
{
    // 1. Cấp xung nhịp
    __HAL_RCC_GPIOA_CLK_ENABLE();
    __HAL_RCC_ADC1_CLK_ENABLE();

    // 2. Chuyển PA5 và PA7 sang Analog
    GPIOA->MODER &= ~((3U << (5 * 2)) | (3U << (7 * 2)));
    GPIOA->MODER |= (3U << (5 * 2)) | (3U << (7 * 2));

    // 3. SỬA LỖI TÊN THANH GHI: Chia tần số an toàn (PCLK2 / 4)
    ADC123_COMMON->CCR &= ~(3U << 16);
    ADC123_COMMON->CCR |= (1U << 16);

    // 4. Kéo dài thời gian lấy mẫu (480 cycles)
    ADC1->SMPR2 &= ~((7U << 15) | (7U << 21));
    ADC1->SMPR2 |= (7U << 15) | (7U << 21);

    // 5. Cấu hình Single mode
    ADC1->CR1 = 0;
    ADC1->CR2 = 0;
    ADC1->SQR1 = 0;

    // 6. Bật nguồn và CHỜ ỔN ĐỊNH (t_STAB)
    ADC1->CR2 |= (1U << 0);
    for(volatile uint32_t wait = 0; wait < 10000; wait++); // Delay giả lập ~vài chục us
}
/**
...
void StartDefaultTask(void *argument)
{
  /* USER CODE BEGIN 5 */
  // ==========================================================
  // 1. KHAI BÁO BIẾN TRẠNG THÁI PHE A (CHỮ HOA)
  // ==========================================================
  uint8_t current_cmd_A = 'S';
  uint8_t last_cmd_A = 'S';
  uint32_t send_timer_A = 0;

  uint8_t last_A_SW = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_2);;
  uint8_t last_A_B = 0;
  uint8_t last_A_Atk = 0;
  uint32_t press_time_A = 0;

  // ==========================================================
  // 2. KHAI BÁO BIẾN TRẠNG THÁI PHE B (CHỮ THƯỜNG)
  // ==========================================================
  uint8_t current_cmd_B = 's';
  uint8_t last_cmd_B = 's';
  uint32_t send_timer_B = 0;

  uint8_t last_B_SW = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_3);;
  uint8_t last_B_B = 0;
  ...
  for(;;)
  {
    ...
    // ==========================================================
    // XỬ LÝ PHE A - JOYSTICK (xy_val[0], xy_val[1]) + BUTTON HYBRID
    // ==========================================================

    // Đọc trạng thái chân Digital Phe A theo sơ đồ
    uint8_t curr_A_SW  = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_2);  // Nút SW Joystick (Ulti)
    uint8_t curr_A_Atk = HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_4);  // Nút Đánh
    ...
    // Xử lý Nút Đỡ Phe A
    if (curr_A_B != last_A_B) {
        data = (curr_A_B == GPIO_PIN_SET) ? 'B' : 'S';
        osMessageQueuePut(Queue1Handle, &data, 0, 10);
        last_A_B = curr_A_B;
    }

    // Xử lý Nút Tấn Công Phe A (Máy trạng thái kết hợp ngữ cảnh di chuyển)
    if (curr_A_Atk != last_A_Atk) {
        if (curr_A_Atk == GPIO_PIN_SET) {
            press_time_A = current_time; // Bắt đầu tính giờ nhấn giữ
        } else {
            uint32_t hold_time = current_time - press_time_A;

            // NGỮ CẢNH ĐÁ: Nếu đang trong trạng thái Cúi ('C') hoặc Nhảy ('J')
            if (current_cmd_A == 'C' || current_cmd_A == 'J') {
                data = (hold_time > 400) ? 'X' : 'K'; // 'X': Đá mạnh, 'K': Đá nhẹ
            }
            // NGỮ CẢNH ĐẤM TAY: Khi đang đứng yên hoặc chạy trái/phải
            else {
                data = (hold_time > 400) ? 'H' : 'A'; // 'H': Đấm mạnh, 'A': Đấm nhẹ
            }
            osMessageQueuePut(Queue1Handle, &data, 0, 10);
        }
    ...
    // Xử lý nút SW kích hoạt Tuyệt chiêu Ultimate Phe A
    if (curr_A_SW != last_A_SW) {
    	osDelay(20);
    	if (curr_A_SW != last_A_SW){
    		if (curr_A_SW == GPIO_PIN_RESET) {
				// Khi vừa NHẤN XUỐNG: Ném mã 'W' để báo hiệu bắt đầu tích lực
				data = 'W';
				osMessageQueuePut(Queue1Handle, &data, 0, 10);
			} else {
				// Khi NHẢ TAY RA (Trở về mức SET): Ném mã 'U' để lao đi đâm kiếm
				data = 'U';
				osMessageQueuePut(Queue1Handle, &data, 0, 10);
			}
			last_A_SW = curr_A_SW;
    ...
    // ==========================================================
    // XỬ LÝ PHE B - JOYSTICK (xy_val[2], xy_val[3]) + BUTTON HYBRID
    // ==========================================================

    // Đọc trạng thái chân Digital Phe B theo sơ đồ mới
    uint8_t curr_B_SW  = HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_3);  // Nút SW Joystick (Ulti)
    uint8_t curr_B_Atk = HAL_GPIO_ReadPin(GPIOD, GPIO_PIN_6);  // Nút Đánh
    ...
    if (current_cmd_B == 's') {
        if (last_cmd_B != 's') {
            osMessageQueuePut(Queue1Handle, &current_cmd_B, 0, 10);
            last_cmd_B = 's';
        }
    } else {
        if ((current_cmd_B != last_cmd_B) || (current_time - send_timer_B >= 50)) {
            osMessageQueuePut(Queue1Handle, &current_cmd_B, 0, 10);
            last_cmd_B = current_cmd_B;
            send_timer_B = current_time;
        }
    }

    // Xử lý Nút Đỡ Phe B
    if (curr_B_B != last_B_B) {
        data = (curr_B_B == GPIO_PIN_SET) ? 'b' : 's';
        osMessageQueuePut(Queue1Handle, &data, 0, 10);
        last_B_B = curr_B_B;
    }

    // Xử lý Nút Tấn Công Phe B (Kết hợp ngữ cảnh di chuyển)
    if (curr_B_Atk != last_B_Atk) {
        if (curr_B_Atk == GPIO_PIN_SET) {
            press_time_B = current_time;
        } else {
            uint32_t hold_time = current_time - press_time_B;

            // NGỮ CẢNH ĐÁ: Nếu đang trong trạng thái Cúi ('c') hoặc Nhảy ('j')
            if (current_cmd_B == 'c' || current_cmd_B == 'j') {
                data = (hold_time > 400) ? 'x' : 'k'; // 'x': Đá mạnh, 'k': Đá nhẹ
            }
            // NGỮ CẢNH ĐẤM TAY: Khi đang đứng yên hoặc chạy trái/phải
            else {
        ...
        last_B_Atk = curr_B_Atk;
    }

    // Xử lý nút SW kích hoạt Tuyệt chiêu Ultimate Phe B
    if (curr_B_SW != last_B_SW) {
    	osDelay(20);
    	if (curr_B_SW != last_B_SW) {
    		if (curr_B_SW == GPIO_PIN_RESET) {
				// Khi vừa NHẤN XUỐNG: Ném mã 'w' để báo hiệu bắt đầu tích lực
				data = 'w';
				osMessageQueuePut(Queue1Handle, &data, 0, 10);
			} else {
