...
void Screen1View::setupScreen()
{
    ...
    // Cấu hình hai đội rồi khởi tạo lại với đúng tham số:
    //   playerA: teamId 0, X=0,   nhìn sang phải
    //   playerB: teamId 1, X=110, nhìn sang trái
    playerA.configure(0, 0, true);
    playerA.initialize();
    playerB.configure(1, 110, false);
    playerB.initialize();
...
void Screen1View::handleTickEvent()
{
    ...
    while (osMessageQueueGetCount(Queue1Handle) > 0)
    {
        ...
        if (res >= 'A' && res <= 'Z') {
            playerA.processCommand(res);
        }
        else if (res >= 'a' && res <= 'z') {
            if (!isPvE) {
                // StickMan::processCommand tự chuyển chữ thường -> chữ hoa theo teamId
                playerB.processCommand(res);
            }
        }
    }
    ...
    int posA = playerA.getX();
    int posB = playerB.getX();
    int distance = abs(posB - posA); // Dùng abs() để lỡ có nhảy xuyên qua nhau vẫn tính đúng

    CharState stateA = playerA.getCurrentState();
    CharState stateB = playerB.getCurrentState();

    bool crouchA = playerA.getIsCrouching();
    bool jumpA = playerA.getIsJumping();
    bool crouchB = playerB.getIsCrouching();
